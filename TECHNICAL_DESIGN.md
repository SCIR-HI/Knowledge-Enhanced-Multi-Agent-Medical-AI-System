...
本文档详细说明知识增强多智能体 AI 诊疗系统的整体架构、知识库构建流程、检索模型训练方法、多智能体协作机制和服务接口实现。相比 GitHub README，本文件更偏向技术实现说明，适合作为 `docs/technical_design.md`、系统白皮书或开发者文档使用。
...
所有模型路径、服务地址、检索阈值、Top-K、生成参数和服务端口均集中维护在 `config.py`。检索模块只读取 `EMBEDDING_MODEL`、`RERANK_MODEL`、`FAISS_INDEX_VERSIONS`、`DEFAULT_FAISS_VERSION`、`RETRIEVER_MAIN_TOPK`、`RETRIEVER_SUB_TOPK`、`RETRIEVER_MIN_SCORE`、`RERANK_TOP_N` 和 `VECTOR_RETRIEVER_TOP_K`。
...
代码仓库不直接托管模型权重、FAISS 索引或知识库文件。当前公开引用仅保留 Reranker 权重和 FAISS 知识库：

| 资源类型 | 说明 | 计划发布位置 |
|---|---|---|
| 医学领域 Reranker 权重 | 基于医学 Query-Document 样本微调后的 Cross-Encoder Reranker | https://huggingface.co/literary123/test_encoder_only_base_bge-reranker-base-noscore_qwen_distill2 |
| 向量知识库 | FAISS 索引、知识条目、元数据映射文件 | https://huggingface.co/datasets/literary123/faiss_index_A_v4 |

部署时 `RERANK_MODEL` 可直接使用 Hugging Face 模型仓库名称；FAISS 知识库需要先从 Hugging Face Dataset 下载到本地，再将 `FAISS_INDEX_PATH` 指向下载后的目录。
